...
var url2 = "mongodb://localhost:27017/usersdb";

//для записи базовой версии массива новостей
...
app.post('/loadBase', function (req, res) {
     mongoClient.connect(url, function(err, db) {
        collection.drop();
        collection.insertMany(req.body, function(err, result){ db.close(); });
    });
    res.json("ALLRIRHT");
    return res;
});
app.get('/article', function (req, res) {
      mongoClient.connect(url, function (err, db) {
        if (req.query.id) {
            db.collection("news").find({id:req.query.id}).toArray(function (err, results) {
                res.json(results);
                db.close();
            });
        }else{
            db.collection("news").find().toArray(function (err, results) {
                var articles2 = [];
                for (var i = 0; i < results.length; i++) if (results[i].isDeleted != 'true') articles2.push(results[i]);
                res.json(articles2);
                db.close();
            });
        }
...
app.get('/article/:id', function (req, res) {
    db.collection("news").find({id:req.params.id}).toArray(function (err, results) {
        res.json(results);
        db.close();
    });
});
app.post('/article', function (req, res) {
    ...
    mongoClient.connect(url, function(err, db) {
        db.collection("news").insert(user, function (err, result) { db.close(); });
        res.json(true);
    });
});
...
app.delete('/article', function (req, res) {
    mongoClient.connect(url, function(err, db) {
        db.collection("news").deleteOne({id: req.body.id}, function(err, result) {
            res.json(true);
            db.close();
        });
    });
...
app.delete('/article/:id', function (req, res) {
    mongoClient.connect(url, function(err, db) {
        db.collection("news").deleteOne({id: req.param.id}, function(err, result) {
            res.json(true);
            db.close();
        });
    });
...
//для записи базового массива пользователей с логинами
