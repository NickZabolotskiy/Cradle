...
function removeArticle(id) {
    for(var i=0; i<articles.length; i++){
        if(articles[i].id == id) articles[i].isDeleted = 'true';
    }
}
...
function saveBase(next,req,res) {
    ft.writeFile("articles.json",JSON.stringify(articles));
    ft.close(2);
    next(req,res);
}
function loadArray(next,req,res) {
...
app.post('/article', function (req, res) {                 // закинуть
    loadArray(function(req,res) {
        let user = {
            id: req.body.id,
        ...
        articles.push(user);
        saveBase(function (res, user) {
            res.json(true); }, res);
    }, req, res);
});
app.put('/article', function (req, res) {                                        // тоже изменить
    loadArray(function(req,res) {
        let user = getArticle(req.body.id);
        user.title = req.body.title;
        user.summary = req.body.summary;
        user.content = req.body.content;
        editArticle(user.id, user);
        saveBase(function (res) {
            res.json(true);
        }, res);
    }, req,res);
});
app.put('/article/:id', function (req, res) {                                        // тоже изменить
    loadArray(function(req,res) {
        let user = getArticle(req.params.id);
        user.title = req.body.title;
        user.summary = req.body.summary;
        user.content = req.body.content;
        editArticle(user.id, user);
...
passport.use(new LocalStrategy({  passReqToCallback : true },
    function(req, username, password, done) {
        ...
        fs.readFile('users.json','utf8', function (err, data) {
            if (!err) {
                if( data != 0 &&  data != 1) users = JSON.parse(data);
                else users = [{user:"admin", password:"admin"},{user:"user", password:"password"}];

                for(var i=0; i<users.length; i++) if(username == users[i].user && users[i].password == password)  return done(null,users[i]);
